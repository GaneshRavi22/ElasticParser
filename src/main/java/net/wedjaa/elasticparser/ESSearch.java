...
public class ESSearch implements Connection {
	 ...
	 * @param searchMode
	 *            the type of results we want to obtain: hits, facets or
	 *            aggregates
	 * 
	...
	public ESSearch(String indexes, String types, int searchMode) {
		this(indexes, types, searchMode, ES_DEFAULT_HOST, ES_DEFAULT_PORT,
				null, null, ES_DEFAULT_CLUSTER);
	}
	...
	public ESSearch(String indexes, String types) {
		this(indexes, types, ES_DEFAULT_SEARCH_MODE, ES_DEFAULT_HOST,
				ES_DEFAULT_PORT, null, null, ES_DEFAULT_CLUSTER);
	}
	...
	public ESSearch(String indexes, String types, String hostname, int port) {
		this(indexes, types, ES_DEFAULT_SEARCH_MODE, hostname, port, null,
				null, ES_DEFAULT_CLUSTER);
	}
	...
	public ESSearch(String indexes, String types, int searchMode,
		...
		this(indexes, types, searchMode, hostname, port, null, null,
				ES_DEFAULT_CLUSTER);
	}
	 ...
	 */
	public ESSearch(String indexes, String types, int searchMode,
			String hostname, int port, String cluster) {
	...
	public ESSearch clone() {
		...
		return new ESSearch(strIndexes, strTypes, searchMode, hostname, port,
	...
	private SearchResponse getQueryCount(String query) {
		...
		SearchResponse searchRes = searchBuilder.setSource(query.getBytes())
				.execute().actionGet();
	...
	private void runQuery(String query, boolean countOnly) {
		...
		case ESSearch.ES_MODE_HITS:
			...
			logger.debug("The query returns " + searchRes.getHits().getTotalHits()
					+ " total matches.");
	...
	public Map<String, Object> next() {
		...
		if (!pager.hit_available()) {
			logger.debug("Getting a page of hits - from: "
					+ pager.current_hit_idx() + ", size: " + pager.page_size());
			// No hits are available - fetch the next
			// page of hits for the query
			searchResponse = executeSearch(pager.get_query(),
					pager.current_hit_idx(), pager.page_size());
			pager.set_page_size(searchResponse.getHits().getHits().length);
			logger.debug("Page hits: " + pager.page_size());
		}

		logger.debug("Returning hit at: " + (pager.current_hit_idx() + 1)
				+ " of " + pager.page_size());
