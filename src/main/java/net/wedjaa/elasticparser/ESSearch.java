...
public class ESSearch implements Connection {
	...
	/**
	 * Create an ESSearch instance initialized to connect to the local node and
	 ...
	 * as a searchMode.
	 * 
	 * @param indexes a comma separated list of indexes that
	 * should be searched. If <em>null</em> all the indexes will be searched.
	 * 
	 * @param types
	 *            a comma separated list of types that should be searched.If
	 *            <em>null</em> all the types will be searched.
	 */
	public ESSearch(String indexes, String types) {
		this(indexes, types, ES_DEFAULT_HOST, ES_DEFAULT_PORT, null, null, ES_DEFAULT_CLUSTER);
	}
	 ...
	 *
	 * @param indexes
	 *            a comma separated list of indexes that should be searched. If
	 *            <em>null</em> all the indexes will be searched.
	 * @param types
	 *            a comma separated list of types that should be searched.If
	 *            <em>null</em> all the types will be searched.
	 * @param hostname
	 *            hostname to connect to - or IP address
	 * @param port
	 *            a port to connect to for trasport. The default transport port
	 *            is 9300.
	 */
	public ESSearch(String indexes, String types, String hostname, int port) {
		this(indexes, types, hostname, port, null, null, ES_DEFAULT_CLUSTER);
	}

	/**
	 * Create a ESSearch object initialized to connect to the specified node and
	 * cluster to search the specified indexes and types, returning
	 * <em>Hits</em>.
	 *
	 * @param indexes
	 *            a comma separated list of indexes that should be searched. If
	 ...
	 *            hostname to connect to - or IP address
	 * @param port
	 *            a port to connect to for transport. The default transport port
	 *            is 9300.
	 * @param cluster
	 *            a cluster name to join. The default cluster name is
	 *            "elasticsearch".
	 */
	public ESSearch(String indexes, String types, String hostname, int port, String cluster) {
		this(indexes, types, hostname, port, null, null, cluster);
	}

	/**
	 * Create a ESSearch object initialized to connect to the specified node and
	 * cluster to search the specified indexes and types, returning hits, facets
	 * or aggregates. This connection handles
	 ...
	 *            is 9300.
	 * @param username
	 *            the username to use to authenticate to ElasticSearch
	 * @param password
	 *            the password of the user specified to connect to ElasticSearch
	 * @param cluster
	 *            a cluster name to join. The default cluster name is
	 *            "elasticsearch".
	 */
	public ESSearch(String indexes, String types, String hostname, int port, String username, String password,
			String cluster) {
	...
	/**
	 * @param esClient
	 *            An already connected ElasticSearch client that can be used to
	 *            execute the queries.
	 *
	 * @returns an instance of ESSearch that can be used to execute ES queries
	 *          and get the results
	 */
	public ESSearch(Client esClient) {
		this.esClient = esClient;
		this.keepClient = true;
	}

	/**
	 * Creates a clone of this ESSearch, keeping the parameters
	 *
	 * @returns a clone of this ESSearch
	 */
	public ESSearch clone() {
		if (esClient != null) {
			return new ESSearch(esClient);
		}
		return new ESSearch(strIndexes, strTypes, hostname, port, username, password, cluster);
	}
	...
	private int getPageSize(String query) {
		/**
		 * If not specified by the user, use a page size we are comfortable
		 * with.
		 */
	...
	private SearchResponse executeSearch(String query, boolean isScrolling) {
		...
		if (isScrolling) {
			...
			searchBuilder.setSearchType(SearchType.SCAN).setScroll(new TimeValue(ESHitsPager.SCROLL_KEEPALIVE));
		...
		// Execute the search
		SearchResponse searchRes = searchBuilder.setSource(query).execute().actionGet();
	...
	private void runQuery(String query, boolean countOnly) {
		...
		if (query.contains("\"facets\"")) {
			searchRes = executeSearch(query);
			pager = new ESFacetsPager(searchRes, query);
		} else if (query.contains("\"aggregations\"") || query.contains("\"aggs\"")) {
			searchRes = executeSearch(query);
	 ...
	 *
	 * @param executor
	 *            The <code>Executor</code> implementation which will be used by
	 *            <code>abort</code>.
	 * @throws java.sql.SQLException
	...
	/**
	 * Sets the maximum period a <code>Connection</code> or objects created from
	 * the <code>Connection</code> will wait for the database to reply to any
	 ...
	 * <code>close</code>, <code>isClosed</code> or
	 * <code>Connection.isValid</code> methods, will result in a
	 ...
	 * administrator thread. The <code>setNetworkTimeout</code> method will
	 ...
	 * object before allowing the method to proceed. If a
	 * <code>SecurityManager</code> exists and its <code>checkPermission</code>
	 * method denies calling <code>setNetworkTimeout</code>, this method throws
	 * a <code>java.lang.SecurityException</code>.
	 *
	 * @param executor
	 *            The <code>Executor</code> implementation which will be used by
	 ...
	 *             the value specified for <code>seconds</code> is less than 0.
	 * @throws SecurityException
	 *             if a security manager exists and its
	 *             <code>checkPermission</code> method denies calling
	 *             <code>setNetworkTimeout</code>.
